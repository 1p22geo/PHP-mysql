...
<body>
    ...
    <section>
        ...
        if (array_key_exists('deleted', $_GET)){
            $del = $_GET['deleted'];
            echo "<h2>Session {$del} expired succesfully</h2>";
        }
        ?>
        <h2>All Sessions:</h2>
        <a href="opensessions.php?session=<?php echo $id?>" id='link'>Show only active sessions</a>
        <table>
            <tr>
                <th>Username</th>
                <th>Opening date</th>
            ...
            <?php 
            $res = query("SELECT * FROM loginsessions ORDER BY start_time DESC");
            if($res->num_rows > 0){
